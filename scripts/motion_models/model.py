...
IntegratorFunction = Callable[[StateDiffFunction, np.ndarray, np.ndarray, float], np.ndarray] # x_next = f(dstate, x, u, dt)
...
def predict(model: MotionModel, x_0: np.ndarray, u_arr: np.ndarray, dt: float, integrator: IntegratorFunction) -> np.ndarray:
    """
